...
const storePost = (req,res)=>{
       ...
       form.parse(req,(err,fields,files)=>{             
        ...
        const imageExt = split[split.length-1].toUpperCase().toUpperCase();
        if(files.image.name.length===0){
            errors.push({msg:"Image is required"})

        }else{
        if(imageExt!=="JPG"  && imageExt!=="PNG"){
                errors.push({msg:"Image extenstion in wrong please choose an appropriate image"});
        }
        }
...
const posts =async (req,res)=>{
    ...
    res.render("Posts",{title:"Posts",login:true,posts:allPosts,format:dateFormat,count,perPage,currentPage})
}
...
module.exports = {deletePost,PostForm,storePost,posts,details,updateForm,postUpdate,postValidations}
